...
.factory('$transition', ['$q', '$timeout', '$rootScope', function($q, $timeout, $rootScope) {
  ...
  /**
   * 'WebkitTransition': 'transitionend', is failing for chrome. 
   * Changed it to transitionend
   * 
  ...
  var transitionEndEventNames = {
    'WebkitTransition': 'transitionend',
    'MozTransition': 'transitionend',
    'OTransition': 'oTransitionEnd',
    'transition': 'transitionend'
  };
...
.controller('DatepickerController', ['$scope', '$attrs', '$parse', '$interpolate', '$timeout', '$log', 'dateFilter', 'datepickerConfig', function($scope, $attrs, $parse, $interpolate, $timeout, $log, dateFilter, datepickerConfig) {
  ...
  angular.forEach(['formatDay', 'formatMonth', 'formatYear', 'formatDayHeader', 'formatDayTitle', 'formatMonthTitle',
                   'minMode', 'maxMode', 'showWeeks', 'startingDay', 'yearRange', 'dateCleared'], function( key, index ) {
    self[key] = angular.isDefined($attrs[key]) ? (index < 8 ? $interpolate($attrs[key])($scope.$parent) : $scope.$parent.$eval($attrs[key])) : datepickerConfig[key];
  });

  // Watchable date attributes
  angular.forEach(['minDate', 'maxDate', 'dateCleared'], function( key ) {
    if ( $attrs[key] ) {
    	if (key != 'dateCleared') {
  ...
  this.refreshView = function() {
    if (this.dateCleared != this.lastCleared) {
    	this.activeDate = angular.isDefined($attrs.initDate) ? $scope.$parent.$eval($attrs.initDate) : new Date();
    	this.lastCleared = this.dateCleared;
	}
    if ( this.element ) {
      this._refreshView();
...
function ($compile, $parse, $document, $position, dateFilter, dateParser, datepickerPopupConfig) {
  return {
    ...
    link: function(scope, element, attrs, ngModel) {
      ...
      var datepickerAttrs = '', allowInvalid = false;
      if(attrs.hasOwnProperty('ngModelOptions') && attrs.ngModelOptions.indexOf('allowInvalid') !== -1) {
    	  var temp = attrs.ngModelOptions.substring(attrs.ngModelOptions.indexOf('allowInvalid'));
    	  temp = temp.substring(temp.indexOf(':') +1).trim().substring(0, 4);
    	  allowInvalid = temp === 'true';
    	  datepickerAttrs = 'ng-model-options="{allowInvalid:' + allowInvalid + '}"';
      }
      // popup element used to display calendar
      var popupEl = angular.element('<div datepicker-popup-wrap><div datepicker ' + datepickerAttrs + '></div></div>');
      popupEl.attr({
        'ng-model': 'date',
        'ng-change': 'dateSelection()'
      });
      if(datepickerAttrs.length > 0) {
    	  popupEl.attr({'ng-model-options': '{allowInvalid: ' + allowInvalid + '}'});
	  }
      ...
      scope.watchData = {};
      angular.forEach(['minDate', 'maxDate', 'datepickerMode', 'dateCleared'], function( key ) {
        if ( attrs[key] ) {
      ...
      ngModel.$parsers.push(minLimitParse);
      ngModel.$formatters.push(minLimitFormat);
      ngModel.$parsers.push(maxLimitParse);
      ngModel.$formatters.push(maxLimitFormat);
